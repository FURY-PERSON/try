...
test.describe('Questions List Page', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Вопросы' })).toBeVisible({ timeout: 15000 });
  ...
  test('shows generate button in header', async ({ page }) => {
    // Use exact name to avoid matching the empty state "Генерировать вопросы" button
    await expect(
  ...
  test('shows table or empty state after loading', async ({ page }) => {
    ...
    const emptyState = page.locator('main').getByText('Нет вопросов');
  ...
  test('table has correct headers when questions exist', async ({ page }) => {
    const table = page.locator('main table');
    if (await table.isVisible()) {
      const headers = ['Тип', 'Категория', 'Сложность', 'Статус', 'Показов', '% правильных'];
  ...
  test('type filter changes selection', async ({ page }) => {
    const selects = page.locator('main select');
...
test.describe('Question Generate Page', () => {
